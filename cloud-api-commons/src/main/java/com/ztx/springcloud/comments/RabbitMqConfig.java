...
public class RabbitMqConfig {
    ...
    public static final String ROUTING_KEY_4 = "queue_one_key_infants";
 ...
 /*   @Bean
    ...
    /**
     * 将消息队列4和交换机4进行绑定,指定队列key4
     */
   /* @Bean
    public Binding binding_three() {
        return BindingBuilder.bind(queueConfig.fourthQueue()).to(exchangeConfig.directExchange()).with(RabbitMqConfig.ROUTING_KEY_4);
    }*/
   /* @Bean
    public Binding binding_fore() {
        return BindingBuilder.bind(queueConfig.fourthQueue()).to(exchangeConfig.topicExchange()).with(RabbitMqConfig.ROUTING_KEY_4);
    }*/
    //声明news队列和交换机绑定关系，并且指定RoutingKey
    @Bean
    public Binding sendMessageTopic(){
        return BindingBuilder.bind(queueConfig.fourthQueue()).to(exchangeConfig.directExchange01()).with(RabbitMqConfig.ROUTING_KEY_4);
    }
